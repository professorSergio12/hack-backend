...
const FORM_TITLE = "Statutory Certificate";
...
/**
 * @param {object} record – lean StatutoryCertificate document
 * @returns {Promise<Buffer>}
 */
export async function generateStatutoryCertificatePdf(record) {
  ...
  const headerCtl = createQhsePdfHeaderController({
    ...
    meta: buildStandardMeta(record, "HR-STAT"),
  });
  ...
  const body = [
    [
      ...
      labelCell("Type of Document"),
      cellText(record.typeOfDocs),
    ],
    [
      labelCell("Year"),
      cellText(record.year),
      labelCell("Validity"),
    ...
    [
      ...
      labelCell("Updated"),
      cellText(formatDate(record.updatedAt)),
    ],
  ];
